...
class PterodactylSourceConnector implements MigrationSourceConnector
{
    ...
    public function testConnection(): void
    {
        try {
            ...
            throw new RuntimeException(
                'HivePanel could not connect to the Pterodactyl panel: ' . $exception->getMessage(),
                previous: $exception,
            );
        }
    ...
    private function fetchPaginatedResources(
        string $endpoint,
        array $parameters = [],
    ): array {
        $resources = [];
        ...
        do {
            try {
                ...
                throw new RuntimeException(
                    'HivePanel lost connection to the Pterodactyl panel while reading '
                    . $endpoint
                    . ': '
    ...
    private function fetchEggLookup(array $servers): array
    {
        ...
        foreach ($pairs as $key => $pair) {
            try {
                ...
                throw new RuntimeException(
                    'HivePanel could not read Pterodactyl Egg #'
                    . $pair['egg_id']
                    . ': '
                    . $exception->getMessage(),
                    previous: $exception,
    ...
    private function assertSuccessfulResponse(Response $response): void
    {
        ...
        $message = match ($response->status()) {
            401 => 'The Pterodactyl API key was rejected. Check the key and update the migration source credentials.',
            403 => 'The Pterodactyl API key does not have permission to use the Application API. Generate an Application API key with the required read permissions.',
            404 => 'The requested Pterodactyl Application API endpoint could not be found. Check the source panel version and URL.',
            429 => 'The Pterodactyl panel is rate limiting HivePanel. Wait a moment and retry discovery.',
            500, 502, 503, 504 => 'The Pterodactyl panel returned a server error. Check the source panel and retry discovery.',
            default => 'The Pterodactyl panel returned HTTP ' . $response->status() . '.',
        };
        ...
        if (filled($apiMessage)) {
            $message .= ' Pterodactyl says: ' . $apiMessage;
        }
    ...
    private function connectionErrorMessage(ConnectionException $exception): string
    {
        ...
        if (
            ...
            return 'HivePanel could not resolve the Pterodactyl hostname. Check the panel URL and DNS configuration.';
        }
        ...
        if (
            ...
            return 'HivePanel could not verify the Pterodactyl SSL certificate. Check the source panel certificate.';
        }
        ...
        if (
            ...
            return 'The connection to Pterodactyl timed out. Check that the source panel is reachable from the HivePanel web server.';
        }

        if (str_contains($lowerMessage, 'connection refused')) {
            return 'The Pterodactyl panel refused the connection. Check its URL, port and firewall.';
        }

        return 'HivePanel could not connect to the Pterodactyl panel: ' . $message;
    }
